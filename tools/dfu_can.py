"""
CCU firmware updater via CAN DFU protocol — Candlelight USB adapter.

Sends a signed firmware binary to the device using the CCU DFU protocol over CAN.
...
DFU_CMD_ID  = 0x7E0   # host → device: REQUEST or COMMIT
DFU_DATA_ID = 0x7E1   # host → device: firmware chunks
DFU_RSP_ID  = 0x7E2   # device → host: status
...
# Must match CCU_DFU_BATCH_SIZE in can_converter_dfu.h.
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="CCU firmware updater via CAN DFU (Candlelight USB adapter)",
    ...
    print("CCU DFU over CAN  (Candlelight / gs_usb)")
